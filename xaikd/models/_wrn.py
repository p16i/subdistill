# from https://pytorch-ood.readthedocs.io/en/v0.2.0/_modules/pytorch_ood/model/wrn.html#WideResNet
...
class WideResNet(nn.Module):
    ...
    def _from_pretrained(self, name: str):
        ...
        if name in urls.keys():
            ...
            raise ValueError(
                f"Unknown model identifier. Possible values are {list(urls)}"
            )
        ...
        if "pixmix" in name:
            ...
            for key in state_dict.keys():
                if "conv_shortcut" in key:
                    new_state_dict[
                        key.replace("conv_shortcut", "convShortcut")
                    ] = state_dict[key]
