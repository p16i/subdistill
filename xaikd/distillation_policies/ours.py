...
@register_policy("basis-bn-scale")
class OrthogonalBasisBatchNormSumNormalizedPolicy(LayerPolicy):
    def __init__(
        self, teacher_dims: int, student_dims: int, device: str, basis: OrthogonalBasis
    ) -> None:
        ...
        class StudentTranformer(nn.Module):
            def __init__(self, k: int, arr_scaling_factors: npt.NDArray):
                super().__init__()
                self.bn = nn.BatchNorm2d(
                    num_features=k, track_running_stats=True, affine=False
                ).to(device)
                self.scaling = (
                    torch.from_numpy(arr_scaling_factors**0.5)
                    .reshape((1, -1, 1, 1))
                    .to(device)
                )

            def forward(self, x: torch.Tensor):
                return self.scaling * self.bn(x)

        self.transformer_student_feats = StudentTranformer(
            k=k, arr_scaling_factors=self.basis.get_scale_factors_for_k(student_dims)
        )
