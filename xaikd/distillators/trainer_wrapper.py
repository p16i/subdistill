...
class LayerwiseKDModelWrapper(pl.LightningModule):
    ...
    def on_train_epoch_end(self) -> None:
        ...
        for lix, policy in enumerate(layer_policies):
            ...
            # Q, _ = torch.linalg.qr()
            Q = student_transformer.rotation.weight.detach().cpu()
            ...
            self.log(f"p99_theta_{layer_name}", torch.quantile(thetas, 0.99))
            self.log(f"p90_theta_{layer_name}", torch.quantile(thetas, 0.90))

            # self.logger.experiment.log(
            #     {"theta": wandb.Histogram(thetas.cpu().numpy())},
